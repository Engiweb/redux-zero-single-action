...
*/
const singleAction = (state, { keys, action = GENERAL_SET, value = undefined }) => {
  const stateKey = keys[0]

  const newState = {
    [stateKey]: state[stateKey] ? JSON.parse(JSON.stringify(state[stateKey])) : undefined
  }
